                    ...
                    for row in range(sheet.nrows):
                        try:
                            ...
                            if sheet.cell(row, 0).value.strip() not in (None, ""):
                                ...
                                activeornor = True
                                if sheet.cell(row, 21).value == 'FALSE':
                                    activeornor = False
                                ...
                                #pos_categ_id = False
                                """
                                if sheet.cell(row, 20).value.strip() in (None, ""):
                                    search_category = self.env['pos.category'].search(
                                        [('name', '=', 'All')], limit=1)
                                    if search_category:
                                        pos_categ_id = search_category.id
                                    else:
                                        skipped_line_no[str(
                                            counter)] = " - POS Category - All not found. "
                                        counter = counter + 1
                                        continue
                                else:
                                
                                search_category = self.env['pos.category'].search(
                                    [('id', '=', sheet.cell(row, 20).value)], limit=1)
                                if search_category:
                                    pos_categ_id = search_category.id
                                else:
                                    skipped_line_no[str(
                                        counter)] = " - POS Category not found. "
                                    counter = counter + 1
                                    continue
                                """
                                ...
                                vals = {
                                    ...
                                    'type': product_type,
                                    'categ_id': categ_id,
                                    'available_in_pos': sheet.cell(row, 19).value,
                                    'pos_categ_id': sheet.cell(row, 20).value,
                                    'active': activeornor,
                            ...
                            skipped_line_no[str(
                                counter)] = " - Value is not valid. " + ustr(e) + " - Error line " + traceback.format_exc()                          
                            counter = counter + 1
                            continue
