...
class DynamicPricelistUpdatedReport(models.Model):
    ...
    date_from = fields.Datetime(string='orderdate From')
    date_to = fields.Datetime(string='orderdate to')
        ...
        if data.get('date_from') and data.get('date_to'):
            ...
            query = \
                '''
                 WITH StockSummary AS (
                    ...
                    total_in AS (
                        SELECT
                            im.product_id,
                            im.location_dest_id AS location_id,
                            SUM(im.qty) AS total_in
                        FROM
                            inventory_moves im
                        WHERE
                        	im.location_dest_id = '{3}' AND
                            im.orddate <= '{0}' 
                        GROUP BY
                            im.product_id, im.location_dest_id
                    ), total_out AS (
                        SELECT
                            im.product_id,
                            im.location_id,
                            -SUM(im.qty) AS total_out
                        FROM
                            inventory_moves im
                        WHERE
                            
                        	im.location_id='{3}' AND
                            im.orddate <= '{0}' 
                        GROUP BY
                            im.product_id, im.location_id
						)
						SELECT 
                        i.product_id,
                        i.location_id,
                ...
                TotalIn AS (
                    ...
                    SELECT
                        product_id,
                        COALESCE(SUM(CASE WHEN (location_dest_id != 14 AND location_dest_id = '{3}') THEN qty ELSE 0 END), 0) AS total_in
                    FROM
                        inventory_moves
                    WHERE
                         orddate >= '{0}' AND orddate <= '{1}'
                    GROUP BY
                        product_id
                ),
                TotalOut AS (
                    ...
                    SELECT
                        product_id,
                        COALESCE(SUM(CASE WHEN (location_dest_id != 14 AND location_id = '{3}') THEN qty ELSE 0 END), 0) AS total_out
                    FROM
                        inventory_moves
                    WHERE
                ...
                UsageSummary AS (
                    ...
                    product_id,
                    SUM(CASE
                        WHEN location_id = 5 THEN qty  
                        ELSE 0
                    END) AS return_from_customer,
                    SUM(CASE
                        WHEN location_dest_id = 5 THEN -qty  
                        ELSE 0
                    END) AS sales_to_customer,
                    SUM(CASE
                        WHEN location_id = 4 THEN -qty  
                        ELSE 0
                    END) AS return_from_supplier,
                    SUM(CASE
                        WHEN location_dest_id = 4 THEN qty  
                        ELSE 0
                    END) AS purchase_from_supplier,
                    SUM(CASE
                        WHEN location_id != 4 AND location_id != 5 AND location_id != 14 AND location_id = '{3}' THEN qty  
                        ELSE 0
                    END) AS transfer_outgoing_location,
                    SUM(CASE
                        WHEN location_dest_id != 4 AND location_dest_id != 5 AND location_dest_id != 14 AND location_dest_id = '{3}' THEN qty  
                        ELSE 0
                ...
                JOIN StockSummary ON product_product.id = StockSummary.product_id
                JOIN TotalIn ON product_product.id = TotalIn.product_id
                JOIN TotalOut ON product_product.id = TotalOut.product_id
                JOIN UsageSummary ON product_product.id = UsageSummary.product_id
                WHERE 
