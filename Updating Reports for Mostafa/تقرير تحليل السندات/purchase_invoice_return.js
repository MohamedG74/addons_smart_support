odoo.define('smart_custom_report.purchase_invoice_return', function(require) {
    'use strict';
    ...
    var PurchaseInvoiceReturn = AbstractAction.extend({
       template: 'PurchaseInvoiceReturn',
       events: {
  ...
  start: function() {
   ...
   $("head").append("<link href='/smart_custom_report/static/src/css/tabulator.css' type='text/css' rel='stylesheet'/>");
    ...
    rpc.query({
       model: 'dynamic.purchase.invoice.return',
       method: 'create',
       args: [{
       }]
    }).then(function(res) {
  ...
  load_data: function(initial_render = true) {
          ...
          self._rpc({
            ...
            if (datas['orders']){
               self.$('.table_view_pr').html(QWeb.render('PurchaseInvoiceReturnOrderTable', {
             ...
             if (initial_render) {
                self.$('.filter_view_pr').html(QWeb.render('PurchaseInvoiceReturnFilterView', {
      ...
      //   });
       ...
       groupButton: function (event) {
         ...
         if (!table) {
               ...
               movableRows: true,
                  columns: [
                     ...
                     { title: "المستخدم", field: "user_name", widthGrow:1 },
         ...
         // // Check if a field is selected for grouping
         // if (selectedField) {
         //       // Group the table by the selected field
         ...
         exportButton.addEventListener("click", function downloadOnce() {
            table.download("xlsx", "تقرير فواتير و مرتجعات المشتريات.xlsx", {sheetName: "Purchase Invoices Returns"});
   ...
   print_pdf: function(e) {
          ...
          return $("#table2excel").table2excel({
              ...
              filename: "Purchase Invoices Returns", //do not include extension
  ...
  core.action_registry.add("p_i_r", PurchaseInvoiceReturn);
    return PurchaseInvoiceReturn;
  });