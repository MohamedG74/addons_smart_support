odoo.define('smart_custom_report.pricelist_inventory_report', function(require) {
   'use strict';
   ...
   var UpdatedPricelistReport = AbstractAction.extend({
      template: 'UpdatedPricelistReport',
      events: {
 ...
 start: function() {
  ...
  $("head").append("<link href='/smart_custom_report/static/src/css/tabulator.css' type='text/css' rel='stylesheet'/>");
   ...
   rpc.query({
      model: 'dynamic.pricelist.inventory.report',
      method: 'create',
      args: [{
      }]
   }).then(function(res) {
 ...
 load_data: function(initial_render = true) {
         ...
         self._rpc({
           ...
           if (datas['orders']){
              self.$('.table_view_pr').html(QWeb.render('UpdatedPricelistOrderTable', {
            ...
            if (initial_render) {
               self.$('.filter_view_pr').html(QWeb.render('UpdatedPricelistFilterView', {
           ...
           if(datas['locations']){
            ...
            if(datas['groups']){
              ...
              $('.second_group').select2({
               ...
               if(datas['stocks']){
               $('.stocks_name').select2('destroy'); 
                 var elms = "";
                 elms = elms + "<option value='all'>All</option>"; 
                 datas['stocks'].forEach(item => {
                    // elms = elms + "<option value='"+item.location_id+"'>"+item.location_name+"</option>";
                    elms = elms + "<option value='"+item.location_name+"'>"+item.location_name+"</option>";  
                  });
                  console.log(elms);
                 $('#selection_stocks_name').html(elms);
                 $('.stocks_name').select2({
      ...
      groupButton: function (event) {
        ...
        if (!table) {
            ...
            table = new Tabulator("#table2excel", {
              ...
              movableRows: true,
                columns: [
                    { title: "المستودع", field: "warehouse_name", widthGrow:1 },
  ...
  print_pdf: function(e) {
         ...
         return $("#table2excel").table2excel({
             ...
             filename: "Pricelist Inventory Report", //do not include extension
  ...
  [false, 'list'],
      ...
      apply_filter: function() {
        ...
        if ($(".stocks_name").length) {
         // var report_res = document.getElementById("report_res_main_group");
         filter_data_selected.stocks_name = $(".stocks_name option:selected").val();
         // report_res.value =  $(".main_group option:selected").val();
         // report_res.innerHTML =  $(".main_group option:selected").text();
         // if ($(".main_group option:selected").val() == "") {
         //    report_res.innerHTML = "None";
 ...
 core.action_registry.add("pl_i_r", UpdatedPricelistReport);
   return UpdatedPricelistReport;
 });