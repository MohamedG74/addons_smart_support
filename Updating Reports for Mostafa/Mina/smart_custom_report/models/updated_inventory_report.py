...
class DynamicUpdatedInventoryReport(models.Model):

    _name = 'dynamic.updated.inventory.report'
    ...
    def purchase_report(self, option):
        ...
        report_values = self.env['dynamic.updated.inventory.report'].search([('id', '=', option[0])])
        data = {'model': self}
    ...
    def get_filter_data(self, option):
        r = self.env['dynamic.updated.inventory.report'].search([('id', '=',option[0])])
        default_filters = {}
        filter_dict = {}
        return filter_dict
                ...
                WITH StockSummary AS (
                    WITH initial_stock AS (
                        SELECT DISTINCT ON (product_id)
                            product_id,
                            qty_done AS opening_stock
                        FROM
                            stock_move_line
                        WHERE
                            reference = 'Product Quantity Updated'
                            AND location_id = 14
                            AND location_dest_id = {3} -- Assuming this is the location_dest_id
                            AND date <= '2023-10-26' -- Replace with your start date
                        ORDER BY
                    ...
                    total_in AS (
                        SELECT
                            product_id,
                            COALESCE(SUM(CASE WHEN location_dest_id = {3} THEN qty_done ELSE 0 END), 0) AS total_in
                        FROM
                            stock_move_line
                        WHERE
                            location_dest_id = {3}
                            AND date > '2023-10-26' -- Replace with your start date
                            AND date <= '{0}' -- Replace with your end date
                        GROUP BY
                            product_id
                    ),
                    total_out AS (
                        SELECT
                            product_id,
                            COALESCE(SUM(CASE WHEN location_id = {3} THEN qty_done ELSE 0 END), 0) AS total_out
                        FROM
                            stock_move_line
                        WHERE
                            location_id = {3}
                    ...
                    FROM
                        initial_stock i
                    LEFT JOIN
                        total_in tin ON i.product_id = tin.product_id
                    LEFT JOIN
                        total_out tout ON i.product_id = tout.product_id
                ),
