...
class DynamicPricelistUpdatedReport(models.Model):

    _name = 'dynamic.pricelist.updated.report'
    ...
    def purchase_report(self, option):
        ...
        report_values = self.env['dynamic.pricelist.updated.report'].search([('id', '=', option[0])])
        data = {'model': self}
        ...
        return {
            'name': 'Updated Pricelist Inventory Report',
    ...
    def get_filter_data(self, option):
        r = self.env['dynamic.pricelist.updated.report'].search([('id', '=',option[0])])
        default_filters = {}
        filter_dict = {}
        return filter_dict
            ...
            query = \
                ...
                WITH StockSummary AS (
                    ...
                    total_in AS (
                        SELECT
                            itimiid,
                            COALESCE(SUM(CASE WHEN loctoidtwo = '{3}' THEN qty ELSE 0 END), 0) AS total_in
                        FROM
                            stock_history
                        WHERE
                            loctoidtwo = {3}
                            AND orderdate > '{0}' -- Replace with your start date
                            AND orderdate <= '{1}' -- Replace with your end date
                        GROUP BY
							itimiid
                    ),
                    total_out AS (
                        SELECT
                            itimiid,
                            COALESCE(SUM(CASE WHEN locfromidtwo = {3} THEN qty ELSE 0 END), 0) AS total_out
                        FROM
                            stock_history
                        WHERE
                            locfromidtwo = {3}
                    ...
                    SELECT
                        i.itimiid,
                        i.opening_stock + COALESCE(tin.total_in, 0) - COALESCE(tout.total_out, 0) AS opening_stock
                    FROM
                        initial_stock i
                    LEFT JOIN
                        total_in tin ON i.itimiid = tin.itimiid
                    LEFT JOIN
                        total_out tout ON i.itimiid = tout.itimiid
                ),
                ...
                TotalIn AS (
                    ...
                    SELECT
                        itimiid,
                        COALESCE(SUM(CASE WHEN (loctoidtwo != 14 AND loctoidtwo = '{3}') THEN qty ELSE 0 END), 0) AS total_in
                    FROM
                        stock_history
                    WHERE
                        (loctoidtwo != 14 AND loctoidtwo = '{3}')
                        AND orderdate >= '{0}' AND orderdate <= '{1}'
                    GROUP BY
                        itimiid
                ),
                TotalOut AS (
                    ...
                    SELECT
                        itimiid,
                        COALESCE(SUM(CASE WHEN (loctoidtwo != 14 AND locfromidtwo = '{3}') THEN qty ELSE 0 END), 0) AS total_out
                    FROM
                        stock_history
                    WHERE
                        (locfromidtwo != 14 AND locfromidtwo = '{3}')
                        AND orderdate >= '{0}' AND orderdate <= '{1}'
                    GROUP BY
                        itimiid
                ),
                UsageSummary AS (
                    ...
                    SELECT
                        itimiid,
                        SUM(CASE
                            WHEN sm.loctoidtwo = '{3}' AND sl.usage = 'customer' THEN qty
                            ELSE 0
                        END) AS return_from_customer,
                        SUM(CASE
                            WHEN sm.locfromidtwo = '{3}' AND sl_dest.usage = 'customer' THEN qty
                            ELSE 0
                        END) AS sales_to_customer,
                        SUM(CASE
                            WHEN sm.locfromidtwo = '{3}' AND sl_dest.usage = 'supplier' THEN -qty
                            ELSE 0
                        END) AS return_from_supplier,
                        SUM(CASE
                            WHEN sm.loctoidtwo = '{3}' AND sl.usage = 'supplier' THEN qty
                            ELSE 0
                        END) AS purchase_from_supplier,
                        SUM(CASE
                            WHEN sm.locfromidtwo = '{3}' AND sl_dest.usage = 'internal' THEN qty
                            ELSE 0
                        END) AS transfer_outgoing_location,
                        SUM(CASE
                            WHEN sm.loctoidtwo = '{3}' AND sl.usage = 'internal' THEN qty
                            ELSE 0
                        END) AS transfer_incoming_location
                    FROM
                        stock_history AS sm
                        INNER JOIN stock_location AS sl
                        ON sm.locfromidtwo = sl.id
                        LEFT JOIN stock_location AS sl_dest
                        ON sm.loctoidtwo = sl_dest.id
                    WHERE
                ...
                SELECT
                    ...
                    (
                        ...
                        JOIN stock_history sl ON sw.id = sl.locfromidtwo
                        JOIN operating_unit ou ON sw.operating_unit_id = ou.id
                        WHERE sl.locfromidtwo = '{3}'
                    ) AS warehouse_name,
                ...
                LEFT JOIN StockSummary ON product_product.id = StockSummary.itimiid
                LEFT JOIN TotalIn ON product_product.id = TotalIn.itimiid
                LEFT JOIN TotalOut ON product_product.id = TotalOut.itimiid
                LEFT JOIN UsageSummary ON product_product.id = UsageSummary.itimiid
                WHERE 
