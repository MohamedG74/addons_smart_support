...
class DynamicItemPurchaseReturn(models.Model):
    ...
    _name = 'dynamic.item.purchase.return'
    
    purchase_report = fields.Char(string='Purchase Report')
    report_product = fields.Char(string='Report Product') 
    invoice_number=fields.Char(string='Invoice Number')
    invoice_type=fields.Selection([('all','all'),('in_invoice','Vendor Bill'),
                                    ('in_refund','Vendor Credit Note'),],string='Invoice Type')
    ...
    def purchase_report(self, option):
        report_values = self.env['dynamic.item.purchase.return'].search([('id', '=', option[0])])
        data = {'model': self}
    ...
    def get_filter_data(self, option):
        r = self.env['dynamic.item.purchase.return'].search([('id', '=',option[0])])
        default_filters = {}
        filter_dict = {}
        return filter_dict
        ...
        if data.get('date_from') and data.get('date_to'):
            ...
            query = \
                ...
                SELECT 
                    ...
                    CASE 
                        WHEN account_move.move_type = 'in_invoice' THEN 'شراء'
                        WHEN account_move.move_type = 'in_refund' THEN 'مرتجع شراء'
                    END AS move_type,
                ...
                FROM 
                    ...
                    JOIN hr_employee ON hr_employee.id = account_move.x_studio_employee 
