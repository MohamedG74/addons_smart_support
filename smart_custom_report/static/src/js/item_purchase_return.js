odoo.define('smart_custom_report.item_purchase_return', function(require) {
    'use strict';
    ...
    var ItemPurchaseReturn = AbstractAction.extend({
       template: 'ItemPurchaseReturn',
       events: {
  ...
  start: function() {
   ...
   $("head").append("<link href='/smart_custom_report/static/src/css/tabulator.css' type='text/css' rel='stylesheet'/>");
    ...
    rpc.query({
       model: 'dynamic.item.purchase.return',
       method: 'create',
       args: [{
       }]
    }).then(function(res) {
  ...
  load_data: function(initial_render = true) {
          ...
          self._rpc({
            ...
            if (datas['orders']){
               self.$('.table_view_pr').html(QWeb.render('ItemPurchaseReturnOrderTable', {
            ...
            if (initial_render) {
                self.$('.filter_view_pr').html(QWeb.render('ItemPurchaseReturnFilterView', {
            ...
            //  }
             ...
             if(datas['products']){
               ...
               datas['products'].forEach(item => {
                  elms = elms + "<option value='"+item.name+"'>"+item.default_code+" - "+item.name+"</option>";            
                });
                console.log(elms);
       ...
       groupButton: function (event) {
             ...
             table = new Tabulator("#table2excel", {
               ...
               movableRows: true,
                 columns: [
                     ...
                     { title: "اسم المورد", field: "partner_name", widthGrow:1 },
                     { title: "نسبة الخصم", field: "discount_percent", widthGrow:1 },

                     { title: "الكمية", field: "quantity", widthGrow:1,  bottomCalc:"sum", bottomCalcParams:{precision:3,}},
                     { title: "الاجمالي", field: "before_discount", widthGrow:1,  bottomCalc:"sum", bottomCalcParams:{precision:3,}},
                     { title: "قيمة الخصم", field: "discount_value", widthGrow:1,  bottomCalc:"sum", bottomCalcParams:{precision:3,}},
                     { title: "الضريبة", field: "price_tax", widthGrow:1,  bottomCalc:"sum", bottomCalcParams:{precision:3,}},
                     { title: "الصافي", field: "price_subtotal", widthGrow:1,  bottomCalc:"sum", bottomCalcParams:{precision:3,}},
                 ],
             });
         }
         ...
         if (selectedFields.length > 0) {
            ...
            exportButton.addEventListener("click", function downloadOnce() {
               table.download("xlsx", "تقرير مشتريات و مرتجعات الاصناف.xlsx", {sheetName: "Items Purchase and Returns"});
   ...
   print_pdf: function(e) {
          ...
          return $("#table2excel").table2excel({
              ...
              filename: "Items Purchase and Returns", //do not include extension
  ...
  core.action_registry.add("i_p_r", ItemPurchaseReturn);
    return ItemPurchaseReturn;
  });