odoo.define('smart_custom_report.invoice_report', function(require) {
   'use strict';
 ...
 load_data: function(initial_render = true) {
         ...
         self._rpc({
           ...
           if (datas['orders']){
              self.$('.table_view_pr').html(QWeb.render('InvoiceTable', {
            ...
            if (initial_render) {
               self.$('.filter_view_pr').html(QWeb.render('InvoiceReportFilterView', {
           ...
           if(datas['locations']){
            ...
            if(datas['departments']){
              ...
              var elms = "";
              elms = elms + "<option value='all'>All</option>"; 
              datas['departments'].forEach(item => {
               if (item.department_id) {
                 elms = elms + "<option value='"+item.id+"'>"+item.department_id+ ' - ' +item.name+"</option>";            
               }
               });
               console.log(elms);
              $('#selection_department_id').html(elms);
  ...
  print_pdf: function(e) {
         ...
         return $("#table2excel").table2excel({
             ...
             filename: "Employee Report", //do not include extension
      ...
      apply_filter: function() {
       ...
       //  }
        ...
        if ($(".department_id").length) {
           // var report_res = document.getElementById("report_res_main_group");
           filter_data_selected.department_id = $(".department_id option:selected").val();
           // report_res.value =  $(".department_id option:selected").val();
           // report_res.innerHTML =  $(".department_id option:selected").text();
           // if ($(".department_id option:selected").val() == "") {
           //    report_res.innerHTML = "None";
           // }
           console.log('main group:', filter_data_selected.department_id)
        }
 ...
 core.action_registry.add("i_e_r", InvoiceReport);
