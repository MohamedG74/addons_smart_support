odoo.define('smart_custom_report.sales_invoice_areas', function(require) {
    'use strict';
    ...
    var SalesInvoiceAreas = AbstractAction.extend({
       template: 'SalesInvoiceAreas',
       events: {
  ...
  start: function() {
   ...
   $("head").append("<link href='/smart_custom_report/static/src/css/tabulator.css' type='text/css' rel='stylesheet'/>");
    ...
    rpc.query({
       model: 'dynamic.sales.invoice.areas',
       method: 'create',
       args: [{
       }]
    }).then(function(res) {
  ...
  load_data: function(initial_render = true) {
          ...
          self._rpc({
            ...
            if (datas['orders']){
               self.$('.table_view_pr').html(QWeb.render('SalesInvoiceAreasOrderTable', {
             ...
             if (initial_render) {
                self.$('.filter_view_pr').html(QWeb.render('SalesInvoiceAreasFilterView', {
      ...
      if (selectedFields.length > 0) {
         ...
         exportButton.addEventListener("click", function downloadOnce() {
            table.download("xlsx", "تقرير مبيعات المناطق.xlsx", {sheetName: "Item Sales Areas"});
   ...
   print_pdf: function(e) {
          ...
          return $("#table2excel").table2excel({
              ...
              filename: "Sales Invoices Areas", //do not include extension
  ...
  core.action_registry.add("s_i_a", SalesInvoiceAreas);
    return SalesInvoiceAreas;
  });