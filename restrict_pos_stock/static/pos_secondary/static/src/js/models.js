    ...
    const shOrderlineModel = (Orderline) => class shOrderlineModel extends Orderline {
        ...
        set_quantity(quantity, keep_price) {
            ...
            if (current_uom == primary_uom) {
                ...
                this.set_current_uom(secondary_uom);
                if (!keep_price && !this.price_manually_set) {
                    this.set_unit_price(this.product.second_price/current_uom.ratio);
                    this.order.fix_tax_included_price(this);
                }
            }
            return set_qty
        ...
        check_if_can_add(product){
            var firstqty = 0;
            var secondqty = 0;
            var orderlines = this.order.orderlines;                
            for(var i = 0; i < orderlines.length; i++){
                if(product == orderlines[i].get_product().default_code){
                    if(orderlines[i].is_secondary){
        ...
        change_current_uom(uom_id) {
            var qtyfound = this.check_if_can_add(this.get_product().default_code);
            if(this.get_unit() == this.get_current_uom()){
                //we are changing to first qty we need to check if there is more than one of firstqty
                if(qtyfound[1] >=1 ){
                    //found dont add
                    return Gui.showPopup('ErrorPopup', {
                        title: 'Error',
                        body: 'تم اضافة المنتج من قبل'
                    });
                } 

            }else{
                //we are changing to second qty we need to check if there is more than one of secondqty
                if(qtyfound[0] >=1 ){
                    //found dont add
                    return Gui.showPopup('ErrorPopup', {
                        title: 'Error',
            ...
            if (this.current_uom == this.get_unit()) {
                ...
                this.set_quantity(1);//this.get_secondary_quantity()
                this.is_secondary = true;
                this.set_unit_price(this.product.second_price/this.current_uom.ratio);
            }
            // this.trigger("change", this);
        }
        get_current_uom() {
