    ...
    class OrderListScreen extends PosComponent {
        setup() {
            ...
            self.env.services.rpc({
                model: "pos.order",
                method: "search_read",
                domain: [['session_id.config_id', '=', this.env.pos.config.id]],
            }).then(function (orders) {
                self.env.pos.db.all_display_order = orders     
                
                //new
                self.env.services.rpc({
                    model: 'pos.order.line',
                    method: 'search_read',
                    domain: [['id', 'in', orders.flatMap(obj => obj.lines)]]
                }).then(function (lines) {
                ...
                self.env.pos.db.all_orders_line(orders.flatMap(obj => obj.lines)); 
