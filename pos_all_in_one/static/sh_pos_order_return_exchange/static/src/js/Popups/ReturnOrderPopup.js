odoo.define("sh_pos_order_return_exchange.popup", function (require) {
    ...
    class ReturnOrderPopup extends AbstractAwaitablePopup {
        ...
        async return_exchange_product() {
            ...
            var order_lines = order_data.lines;

            var total_discount = 0;
            var total_subtotal = 0;

            order_lines.forEach(function(line_id) {
            var line_data = self.env.pos.db.order_line_by_id[line_id];
            if (line_data.display_name === 'Discount') {
                total_discount += line_data.price_subtotal;
            } else {
                total_subtotal += line_data.price_subtotal;
            }
            });
            ...
            if (total_discount !== 0) {
              var percentage = total_discount / total_subtotal * 100;
              rounded_percentage = Math.round(percentage * 100) / 100;
            }
            ...
            _.each(self.return_line, async function (each_line) {
                ...
                if(rounded_percentage > 0){
                    current_order.add_product(product, {
                        ...
                        discount: each_line.discount+rounded_percentage,
                    });
                }else{
                    current_order.add_product(product, {
                        quantity: -each_line.qty,
                ...
                if (order_data && order_data.partner_id && order_data.partner_id[0]) {
                    await self.env.pos._loadPartners([order_data.partner_id[0]]);
                    self.env.pos.get_order().set_partner(self.env.pos.db.get_partner_by_id(order_data.partner_id[0]));
                }
                if (order_data && order_data.payment_ids && order_data.payment_ids[0]) {
                    //self.env.pos.get_order().add_paymentline(self.env.pos.payment_methods_by_id[order_data.payment_data[0].payment_method_id[0]]);
                    var method = await self.env.services.rpc({
                        model: "pos.payment",
                        method: "search_read",
                       domain: [['id', '=', order_data.payment_ids[0]]]
                    });
                    current_order.old_method = method[0].payment_method_id[0];
                    var methodtw = await self.env.services.rpc({
                        model: "pos.payment.method",
                        method: "search_read",
                       domain: [['id', '=', current_order.old_method]]
                    });
                    if(methodtw[0]){
                        current_order.old_method_type = methodtw[0].x_studio_type;
                    }

                }
                if (order_data && order_data.payment_data && order_data.payment_data[0]) {
                    //self.env.pos.get_order().add_paymentline(self.env.pos.payment_methods_by_id[order_data.payment_data[0].payment_method_id[0]]);
                    current_order.old_method = order_data.payment_data[0].payment_method_id[0];
                    var method = await self.env.services.rpc({
                        model: "pos.payment.method",
                        method: "search_read",
                       domain: [['id', '=', current_order.old_method]]
                    ...
                    if(method[0]){
                        current_order.old_method_type = method[0].x_studio_type;
                    }
        ...
        updateReturnQty(event) {
            ...
            if (self.env.pos.get_order().is_return && !self.env.pos.config.sh_return_more_qty) {
                if (event.currentTarget.value) {
                    if (parseInt(event.currentTarget.value) > parseInt(event.currentTarget.closest("tr").children[3].innerText)) {
