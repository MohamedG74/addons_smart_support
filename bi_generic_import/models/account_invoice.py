...
class gen_inv(models.TransientModel):
    ...
    def make_invoice(self, values):
        ...
        if invoice_search:
            ...
            inv_due_date = self.find_invoice_date(values.get('due_date'))
            ...
            inv_id = invoice_obj.create({
                ...
                'invoice_name': values.get('invoice'),
                #'thousandcubic': values.get('thousandcubic'),
    ...
    def find_partner(self, name):
        ...
        partner_search = partner_obj.search([('client_number', '=', name)])
        if partner_search:
            return partner_search[0]
        else:
        ...
        if self.import_option == 'csv':
            ...
            for row_no in range(sheet.nrows):
                ...
                if row_no <= 0:
                    ...
                    if line[13]:
                        a1 = int(float(line[13]))
                        a1_as_datetime = datetime(*xlrd.xldate_as_tuple(a1, workbook.datemode))
                        due_date_string = a1_as_datetime.date().strftime('%Y-%m-%d')
                    else:
                        due_date_string = False
                    values.update({'invoice': str(line[0]).replace(".0",""),
                                   'customer': str(line[1]).replace(".0",""),
                                   'client_number': str(line[1]).replace(".0",""),
                                   'currency': line[2],
