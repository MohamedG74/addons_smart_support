...
class gen_sale(models.TransientModel):
	...
	def make_sale(self, values):
		...
		if sale_search:
			...
			order_date = values.get('date') #self.make_order_date(values.get('date'))
			wh = self.find_wh(values.get('warehouse'))
			sale_id = sale_obj.create({
				...
				'sale_name' : values.get('order'),
				'warehouse_id' : wh.id,
				'x_studio_branch' : values.get('branch'),
				'is_import' : True
			})
						...
						if many2x_fields.id:
							if many2x_fields.ttype in ['many2one','many2many']:
								if many2x_fields.ttype =="many2one":
									if values.get(i):
										fetch_m2o = self.env[many2x_fields.relation].search([('id','=',values.get(i))])
										if fetch_m2o.id:
											sale_id.update({
	...
	def make_order_line(self, values, sale_id):
		...
		if self.import_prod_option == 'barcode':
		  product_search = product_obj.search([('id',  '=',values['product'])],limit=1)
	...
	def find_wh(self, name):
		user_obj = self.env['stock.warehouse']
		user_search = user_obj.search([('name', '=', name)],limit=1)
		if user_search:
			return user_search
		else:
			raise ValidationError(_(' "%s" WH is not available.') % name)
	...
	def find_partner(self, name):
		...
		partner_search = partner_obj.search([('id', '=', name)])
		if partner_search:
			...
			for row_no in range(sheet.nrows):
				...
				if row_no <= 0:
					...
					if line[10] != '':
						...
						#		raise ValidationError(_('Wrong Date Format. Date Should be in format YYYY-MM-DD.2'+str(len(line[10]))))					
						#a1 = int(float(line[10]))
						#a1_as_datetime = datetime(*xlrd.xldate_as_tuple(a1, 0))
						#date_string = a1_as_datetime.date().strftime('%Y-%m-%d')
						date_string = str(line[10]) + " 00:00:00"
					else:
						raise ValidationError(_("Please assign date."))
