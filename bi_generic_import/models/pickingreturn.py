...
class import_pickingss(models.TransientModel):
    ...
    def create_picking(self, values):
        ...
        if picking_search:
            ...
            picking_type = self.find_pickingtype(str(values.get("location_id")).strip().replace(".0",""))
            ...
            vals = {
                ...
                "location_id": 5,#picking_type.default_location_src_id.id,
    ...
    def find_pickingtype(self, name):
        ...
        partner_search = partner_obj.search([("id", "=", name)],limit=1)
        if partner_search:
            return partner_search
        else:
            raise ValidationError(_("Wrong Picking Name " + name))
    ...
    def import_picking(self):
        ...
        if self.import_option == "csv":
            try:
                keys = ["name", "customer", "origin", "date", "product", "quantity", "lot","branch","location_id"]
