...
class import_pickingss(models.TransientModel):
    ...
    def create_picking(self, values):
        ...
        picking_search = picking_obj.search([("name", "=", str(values.get("name")).strip())])
        pick_id = False
        if picking_search:
    ...
    def make_picking_line(self, values, pick_id):
        ...
        if self.import_prod_option == "barcode":
            product_id = product_obj.search([("id", "=", str(values.get("product")).strip().replace(".0",""))], limit=1)
    ...
    def find_partner(self, name):
        ...
        partner_search = partner_obj.search([("id", "=", str(name).strip().replace(".0",""))])
        if partner_search:
