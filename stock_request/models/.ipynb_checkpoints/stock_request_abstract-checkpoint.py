...
class StockRequest(models.AbstractModel):
    ...
    src_warehouse_id = fields.Many2one(
        "stock.warehouse", "Source Warehouse", ondelete="cascade", required=True
    )
    src_location_id = fields.Many2one(
        "stock.location", "Source location", ondelete="cascade", required=True
    )
    ...
    @api.depends("product_id", "warehouse_id", "location_id","src_warehouse_id","src_location_id")
    def _compute_route_ids(self):
        route_obj = self.env["stock.location.route"]
        routes = route_obj.search(
            [("warehouse_ids", "in", self.mapped("src_warehouse_id").ids)]
        )
        routes_by_warehouse = {}
        for route in routes:
