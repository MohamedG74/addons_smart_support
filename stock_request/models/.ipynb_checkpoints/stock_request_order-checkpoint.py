...
class StockRequestOrder(models.Model):
    ...
    def _def_proc_group(self):
        sequence = self.env['ir.sequence'].search([('code','=','stock.request.order')])
        next= sequence.get_next_char(sequence.number_next_actual)
        current = self.env["procurement.group"].search([('name','=',next)])
        if(current):
            return current
        else:
            return self.env["procurement.group"].create({'name':next})
    ...
    def onchange_src_warehouse_id(self):
        if self.src_warehouse_id:
            # search with sudo because the user may not have permissions
            loc_wh = self.src_location_id.warehouse_id
            if self.src_warehouse_id != loc_wh:
                self.src_location_id = self.src_warehouse_id.lot_stock_id
                self.with_context(no_change_childs=True).onchange_src_location_id()
            if self.src_warehouse_id.company_id != self.company_id:
                self.company_id = self.src_warehouse_id.company_id
                self.with_context(no_change_childs=True).onchange_company_id()
        self.change_childs()
    ...
    def onchange_src_location_id(self):
        if self.src_location_id:
            loc_wh = self.src_location_id.warehouse_id
            if loc_wh and self.src_warehouse_id != loc_wh:
                self.src_warehouse_id = loc_wh
                self.with_context(no_change_childs=True).onchange_src_warehouse_id()
        self.change_childs()        
    ...
    def change_childs(self):
        if not self._context.get("no_change_childs", False):
            for line in self.stock_request_ids:
                ...
                line.src_warehouse_id = self.src_warehouse_id
                line.src_location_id = self.src_location_id
    ...
    def action_accept(self):
        self.mapped("stock_request_ids").action_accept()
        self.write({"state": "open"})
        return True
